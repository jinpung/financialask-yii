...
class Education extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('adviserId, title, year, institution', 'required'),
			array('adviserId, year', 'numerical', 'integerOnly'=>true),
			array('title, institution', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, adviserId, title, year, institution', 'safe', 'on'=>'search'),
		);
	}
